...
it('logs a label_added activity when a label is added', function (): void {
    $ws = Workspace::factory()->create();
    $this->actingInWorkspace($ws);
    $user = User::factory()->for($ws, 'workspace')->create(['email_verified_at' => now(), 'is_developer' => true]);
    $team = Team::factory()->for($ws, 'workspace')->create();
    ...
    $this->withToken($token)->putJson("/v1/issues/{$issue->id}/labels", ['label_ids' => [$label->id]])
        ->assertStatus(200);

    $act = IssueActivity::where('issue_id', $issue->id)->where('type', 'label_added')->first();
    expect($act)->not->toBeNull();
    expect($act->to_value)->toBe($label->name);

    Workspace::forgetCurrent();
});

it('logs a label_removed activity when a label is removed, and none for unchanged labels', function (): void {
    $ws = Workspace::factory()->create();
    $this->actingInWorkspace($ws);
    $user = User::factory()->for($ws, 'workspace')->create(['email_verified_at' => now(), 'is_developer' => true]);
    $team = Team::factory()->for($ws, 'workspace')->create();
    ...
    // start with the label applied
    $this->withToken($token)->putJson("/v1/issues/{$issue->id}/labels", ['label_ids' => [$label->id]])->assertStatus(200);
    // then remove all labels
    $this->withToken($token)->putJson("/v1/issues/{$issue->id}/labels", ['label_ids' => []])->assertStatus(200);

    expect(IssueActivity::where('issue_id', $issue->id)->where('type', 'label_removed')->where('to_value', $label->name)->exists())->toBeTrue();

    // re-setting an already-empty set logs nothing new:
    $before = IssueActivity::where('issue_id', $issue->id)->count();
    $this->withToken($token)->putJson("/v1/issues/{$issue->id}/labels", ['label_ids' => []])->assertStatus(200);
    expect(IssueActivity::where('issue_id', $issue->id)->count())->toBe($before);
