...
const labelStyle: CSSProperties = { display: 'flex', flexDirection: 'column', gap: 6, fontSize: 12, fontWeight: 500, color: 'var(--fg2)' };
const inputStyle: CSSProperties = { height: 42, borderRadius: 9, border: '1px solid var(--border2)', background: 'var(--bg2)', color: 'var(--fg)', fontSize: 14, padding: '0 13px' };
const errStyle: CSSProperties = { margin: 0, fontSize: 12.5, color: 'var(--red)' };
        ...
        <div style={{ display: 'flex', height: '100vh', width: '100%', overflow: 'hidden', color: 'var(--fg)', background: 'var(--bg)' }}>
            ...
            <section style={{ flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 26, padding: 40, overflowY: 'auto' }}>
                <div className="login-fade" style={{ width: '100%', maxWidth: 372 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 11, marginBottom: 38 }}>
                        <div style={{ width: 34, height: 34, borderRadius: 9, background: 'linear-gradient(135deg,var(--accent),#9d7bff)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 700, fontSize: 17, color: '#fff', boxShadow: '0 4px 14px var(--accent2)' }}>P</div>
                        <span style={{ fontSize: 16, fontWeight: 600, letterSpacing: '-.01em' }}>Prizy</span>
                    </div>

                    <h1 style={{ fontSize: 26, fontWeight: 600, letterSpacing: '-.02em', margin: '0 0 7px' }}>Welcome back</h1>
                    <p style={{ margin: '0 0 30px', color: 'var(--fg2)', fontSize: 13.5 }}>Sign in to your workspace to keep things moving.</p>
                    ...
                    <form onSubmit={submit} style={{ display: 'flex', flexDirection: 'column', gap: 15 }}>
                        ...
                        <button type="submit" disabled={login.isPending} className="login-submit" style={{ height: 44, borderRadius: 9, border: 'none', background: 'linear-gradient(135deg,var(--accent),#8a76ff)', color: '#fff', fontSize: 14, fontWeight: 600, cursor: 'pointer', marginTop: 4, boxShadow: '0 6px 18px var(--accent2)', opacity: login.isPending ? 0.6 : 1 }}>
                            {login.isPending ? 'Signing in…' : 'Sign in →'}
                        </button>
                    </form>

                    <p style={{ margin: '26px 0 0', textAlign: 'center', color: 'var(--fg2)', fontSize: 13 }}>New to Prizy? <Link to="/signup" style={{ color: 'var(--accent)' }}>Request access</Link></p>
                </div>
                <div style={{ flexShrink: 0, textAlign: 'center', color: 'var(--fg3)', fontSize: 11 }}>© 2026 Prizy · Privacy · Terms</div>
            </section>
            ...
            <aside className="login-aside" aria-hidden="true" style={{ flex: 1, minWidth: 0, position: 'relative', overflow: 'hidden', background: 'radial-gradient(120% 120% at 80% 10%,#1a1730 0%,#0d0d14 55%,#08080b 100%)', borderLeft: '1px solid var(--border)', display: 'flex', flexDirection: 'column', justifyContent: 'center', padding: 64 }}>
                ...
                <div className="login-fade" style={{ position: 'relative', maxWidth: 440 }}>
                    <div style={{ display: 'inline-flex', alignItems: 'center', gap: 7, padding: '5px 11px', borderRadius: 20, border: '1px solid var(--border2)', background: 'rgba(255,255,255,.03)', fontSize: 11, fontWeight: 500, color: 'var(--fg2)', marginBottom: 26, fontFamily: 'var(--font-mono)' }}>
                        <span style={{ width: 6, height: 6, borderRadius: '50%', background: 'var(--green)', boxShadow: '0 0 8px var(--green)' }} />All systems operational
                    </div>
                    <h2 style={{ fontSize: 33, fontWeight: 600, letterSpacing: '-.025em', lineHeight: 1.2, margin: '0 0 16px' }}>Where support tickets and engineering issues finally live together.</h2>
                    <p style={{ fontSize: 15, color: 'var(--fg2)', lineHeight: 1.6, margin: '0 0 34px' }}>Prizy connects your inbox to your backlog — every escalation traceable from first reply to shipped fix.</p>
                    <div style={{ border: '1px solid var(--border)', borderRadius: 13, background: 'rgba(21,21,25,.72)', backdropFilter: 'blur(8px)', overflow: 'hidden', boxShadow: '0 20px 60px rgba(0,0,0,.4)' }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 7, padding: '11px 15px', borderBottom: '1px solid var(--border)' }}>
                            ...
                            <span style={{ marginLeft: 8, fontSize: 11, color: 'var(--fg3)', fontFamily: 'var(--font-mono)' }}>escalations · live</span>
                        </div>
                        {ROWS.map((r, i) => (
                            <div key={r.id} className="login-row" style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '12px 15px', borderBottom: '1px solid var(--border)', animationDelay: `${i * 0.5}s` }}>
                                <span style={{ fontFamily: 'var(--font-mono)', fontSize: 11, color: 'var(--fg3)', width: 52 }}>{r.id}</span>
                                <span style={{ width: 7, height: 7, borderRadius: '50%', flexShrink: 0, background: r.color, boxShadow: `0 0 7px ${r.color}` }} />
                                <span style={{ flex: 1, fontSize: 12.8, color: 'var(--fg)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{r.title}</span>
                                <span style={{ fontSize: 10.5, fontWeight: 600, padding: '2px 8px', borderRadius: 20, color: r.color, background: `color-mix(in srgb, ${r.color} 16%, transparent)` }}>{r.status}</span>
                            </div>
                        ))}
                    </div>
                </div>
