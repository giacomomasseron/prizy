    ...
    return (
        ...
        <div className="mx-auto max-w-4xl p-6">
            <div className="mb-4 flex items-center justify-between">
                ...
                <Link to="/board" className="text-sm text-indigo-600">Board →</Link>
            </div>
            ...
            <form onSubmit={submit} className="mb-4 flex gap-2">
                ...
                <button type="submit" disabled={!teamId || createIssue.isPending}
                    className="rounded bg-indigo-600 px-3 text-white disabled:opacity-50">Add issue</button>
            </form>
            ...
            <ul className="divide-y rounded border bg-white">
                {data?.items.map((issue) => (
                    <li key={issue.id} className="flex items-center justify-between px-4 py-2">
                        ...
                        <span className="text-xs text-gray-500">{issue.status} · {issue.priority}</span>
                    </li>
                ))}
            </ul>
        </div>
