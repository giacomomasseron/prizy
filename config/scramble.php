...
return [
    ...
    // Intentionally omitting the 'web' middleware group here: that group has
    // NeedsTenant + EnsureValidTenantSession appended globally (bootstrap/app.php),
    // which would abort with NoCurrentTenant on every docs request.  The docs
    // endpoints are read-only, stateless, and do not need session cookies —
    // so 'web' is unnecessary.  RestrictedDocsAccess alone enforces the
    // environment/gate guard (see AppServiceProvider::boot for the gate definition).
    'middleware' => [
